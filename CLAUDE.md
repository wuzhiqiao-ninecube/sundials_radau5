...
- **E2 is n×n complex** — solved via custom SUNDIALS-compliant complex modules: `SUNMatrix_ComplexDense`/`ComplexBand`/`ComplexSparse` and `SUNLinSol_ComplexDense`/`ComplexBand`/`ComplexSparse`, using C99 `double _Complex` as the base element type. Internally these call LAPACK `zgetrf`/`zgetrs` (dense), `zgbtrf`/`zgbtrs` (band), or KLU `klu_z_factor`/`klu_z_solve` (sparse). The 2×2 real block `[[a00, a01],[a10, a11]]` (with `a00=a11`, `a01*a10 < 0`) is reduced to a single complex system via the transform `ω = sqrt(-a01/a10)`, `γ = a00 + i·ω·a10`. RHS packing: `d = b_re + i·ω·b_im`. Solution unpacking: `x_re = Re(z)`, `x_im = Im(z)/ω`. For eigenvalue mode `ω=1`; for Schur mode `ω` varies per pair. One complex E2 per eigenvalue pair: `E2c_mat[0..npairs-1]`.
...
- **Sparse M with sparse J**: When both M and J are sparse, E1's sparsity pattern is the union of J and M, computed eagerly in `Radau5SetLinearSolver(mem, J, M)` via `radau5_SparseUnion`. The complex E2 uses the same n×n pattern (union or J-only) via `SUNMatNewFromPattern_ComplexSparse`. The M parameter is only needed for sparse pattern computation; pass NULL when J is dense/band or when there is no mass matrix.
- **Band J + Band M**: E1/E2 bandwidth is `max(bw_J, bw_M)`. The solver stores `jac_id` (J's actual matrix type) separately from `mat_id` (E1/E2 storage format) to correctly read J even when E1/E2 have wider bandwidth.
- **Band J + Dense M**: When J is band but M is dense, E1/E2 are automatically promoted to dense storage (`mat_id = SUNMATRIX_DENSE`). The solver still reads J as band via `jac_id = SUNMATRIX_BAND`.
...
| `nvector/complex_serial/` | Complex N_Vector (C99 `double _Complex`): all standard ops + complex-specific Pack/Unpack/ComplexScale |
| `sunmatrix/complex_dense/` | Complex dense SUNMatrix: column-major `suncomplextype` storage |
| `sunmatrix/complex_band/` | Complex band SUNMatrix: LAPACK-compatible band storage |
| `sunmatrix/complex_sparse/` | Complex CSC sparse SUNMatrix: KLU-compatible layout |
| `sunlinsol/complex_dense/` | Complex dense linear solver: LAPACK `zgetrf`/`zgetrs` |
| `sunlinsol/complex_band/` | Complex band linear solver: LAPACK `zgbtrf`/`zgbtrs` |
| `sunlinsol/complex_sparse/` | Complex sparse linear solver: KLU `klu_z_factor`/`klu_z_solve` |
