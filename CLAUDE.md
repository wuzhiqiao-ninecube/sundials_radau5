...
- **E2 is n×n complex** — solved directly via LAPACK `zgetrf`/`zgetrs` (dense), `zgbtrf`/`zgbtrs` (band), or KLU `klu_z_factor`/`klu_z_solve` (sparse), bypassing SUNDIALS' real-only SUNLinearSolver interface. The 2×2 real block `[[a00, a01],[a10, a11]]` (with `a00=a11`, `a01*a10 < 0`) is reduced to a single complex system via the transform `ω = sqrt(-a01/a10)`, `γ = a00 + i·ω·a10`. RHS packing: `d = b_re + i·ω·b_im`. Solution unpacking: `x_re = Re(z)`, `x_im = Im(z)/ω`. For eigenvalue mode `ω=1`; for Schur mode `ω` varies per pair. One complex E2 per eigenvalue pair: `E2c_data[0..npairs-1]`. For band matrices, E2 uses LAPACK band storage format with `ldab = 2*ml + mu + 1` (including fill-in rows for LU factorization).
