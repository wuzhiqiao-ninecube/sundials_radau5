...
static int rhs_tba(sunrealtype t, N_Vector y, N_Vector yd, void* ud)
{
  ...
  if (ierr == -1) {
      printf("Error in FCN: illegal input value at time = %g\n", t);
...
int main(int argc, char* argv[])
{
  ...
  /* -----------------------------------------------------------------------
   * Segmented integration: 63 discontinuities at t = 5, 10, ..., 315
   * The five PULSE input signals have derivative jumps at multiples of 5.
   * We integrate segment-by-segment [0,5], [5,10], ..., [310,315] and
   * call Radau5ResetForDiscontinuity at each boundary to reset solver state.
   * ----------------------------------------------------------------------- */
  int ndisc = 63;
  sunrealtype t_end = 5.0 * (sunrealtype)(ndisc + 1);  /* 320 */

  for (int seg = 0; seg <= ndisc; seg++) {
    sunrealtype t_seg_end = 5.0 * (sunrealtype)(seg + 1);

    ret = Radau5Solve(mem, t_seg_end, yout, &tret);

    /* Debug: print first few segments and last */
    if (seg <= 2 || t_seg_end >= 320.0) {
      sunrealtype* yd_dbg = N_VGetArrayPointer(yout);
      printf("[seg %2d] t=%.1f ret=%d tret=%.6e\n",
             seg, t_seg_end, ret, tret);
      /* Print first 5 charge and first 5 voltage components */
      printf("  charges: y[0..4] = %.10e %.10e %.10e %.10e %.10e\n",
             yd_dbg[0], yd_dbg[1], yd_dbg[2], yd_dbg[3], yd_dbg[4]);
      printf("  voltages: y[175..179] = %.10e %.10e %.10e %.10e %.10e\n",
             yd_dbg[175], yd_dbg[176], yd_dbg[177], yd_dbg[178], yd_dbg[179]);
      printf("  outputs: y[223]=%.10e y[304]=%.10e y[322]=%.10e\n",
             yd_dbg[223], yd_dbg[304], yd_dbg[322]);
      /* Check DAE consistency: y[i] - GCN(U)[i] for a few algebraic vars */
      sunrealtype Ucheck[175], Gcheck[175];
      for (int k = 0; k < 175; k++) Ucheck[k] = yd_dbg[k + 175];
      GCN(175, Ucheck, Gcheck, &params);
      printf("  DAE residual: y[0]-g[0]=%.3e y[1]-g[1]=%.3e y[174]-g[174]=%.3e\n",
             yd_dbg[0] - Gcheck[0], yd_dbg[1] - Gcheck[1],
             yd_dbg[174] - Gcheck[174]);
    }
    ...
    /* Reset solver state at discontinuity boundary */
    Radau5ResetForDiscontinuity(mem, h0);
  }

  /* Reference solution at t=315 from Fortran testset (radau5, rtol=atol=1e-5)
